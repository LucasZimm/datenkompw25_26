...
public:
  ...
  void encodeSample  ( PGMImage::Sample s, int k );
  void finish        ()    
  { 
    aenc.finish(); 
  }
...
public:
  ...
  PGMImage::Sample  decodeSample  (int k);

private:
  ArithmeticDecoder adec;
};

void EntropyEncoder::encodeSample( PGMImage::Sample s, int r )
{
  r=0;
...
PGMImage::Sample EntropyDecoder::decodeSample(int r)
{
  r=0;
  int k = r;
  PGMImage::Sample  s      = 0;
  ...
  s = static_cast<PGMImage::Sample>((pre << k) + suf);
  if( s && adec.decBit() )
    s = -s;
  return s;
}
  ...
  void subtractPrediction(EntropyEncoder& eenc)
  {
    ...
    for( int row = 0; row < m_height; row++, line+=m_width, orgLine+=m_width )
      for( int col = 0; col < m_width; col++ ) {
        ...
        int k = codeSegmentA10(contexts[c_Nr].N, contexts[c_Nr].A);
        eenc.encodeSample(static_cast<PGMImage::Sample>(Errval),k);
...
int main() {
    ...
    std::string outputFolder = "output\\losslessImageJPEG-LSRice";
