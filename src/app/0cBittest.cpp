...
void EntropyEncoder::encodeSample(PGMImage::Sample s, unsigned ctxIdx, unsigned sigIdx, int a_err, int b_err, int c_err, int d_err)
{
    ...
    sigIdx = 0;
    aenc.encBin(m_pmfSigCtx[0], nonZero);
    if (!nonZero) return;
    ...
    if (gt1) {
        ...
        while (rem--)
            aenc.encBin(m_pmfAbsCtx[0][0], 1);
        aenc.encBin(m_pmfAbsCtx[0][0], 0);
    }
}
                                               ...
                                               int a_err, int b_err, int c_err, int d_err)
{
    ...
    unsigned gt1Idx = get_gt1_index(a_err, b_err, c_err, d_err);
    gt1Idx = 0;
    bool gt1 = adec.decBin(m_pmfGt1Ctx[0]);
    ...
    if (gt1) {
        mag = 2;
        while (adec.decBin(m_pmfAbsCtx[0][0]))
            mag++;
    }
